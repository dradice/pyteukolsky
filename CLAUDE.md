...
The repository is at an early stage. Currently it contains:
...
- `README.md` — formalism summary and the mode equation the solver evolves.

The numerical evolution code does not exist yet. `README.md` contains the
detailed implementation plan (planned package `pyteukolsky/`: `Grid`,
`TeukolskyRHS`, `Evolution`, diagnostics, initial data) — consult it before
starting implementation. The chosen scheme is method-of-lines (first-order-in-
time reduction with `v = psi_t`, so the mixed `psi_tr` term becomes an explicit
spatial derivative `v_r`) integrated with RK4, on a log-stretched radial grid
and a staggered grid in `mu = cos(theta)`. In `mu` the angular operator is the
Legendre operator `d/dmu[(1-mu^2) d/dmu]` and the potential is
